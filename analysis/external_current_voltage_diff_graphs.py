...
file_1 = "/home/noam/studies/physics/lab_c/sc.git/data/150516/THI_manThermalAutoMeas_9_64.csv"
...
for i in range(len(files)):
    ...
    def plot_v_mean_std(I_ext, V, V_mean_on, V_std_on, V_mean_off, V_std_off):
        if IS_PLOT:        
            ...
            plt.xlabel('external current [amp]')
            plt.ylabel(r'voltage on sample [$\mu$ V]')
            plt.legend(loc='best')
            
    if IS_PLOT:    
        plot_v_mean_std(I_ext, V, V_mean_on, V_std_on, V_mean_off, V_std_off)
        plt.figure()
        plt.title('raw Voltage measurements')
        plt.plot(table['t'][is_each_I_ext_off], V[is_each_I_ext_off], '.b', label='external_current off')
        plt.plot(table['t'][is_each_I_ext_on], V[is_each_I_ext_on], '.r', label='external_current on')
    ...
    if IS_PLOT:
        plt.plot(table['t'][slice_no_edges][::wrap_d], means, 'g', label='voltmeter drift')
        plt.legend(loc='best')
        plt.ylabel(r'V[$\mu$ V]')
        plt.xlabel('time [sec]')
    ...
    plt.title(r'Voltage measurements after drift detranding. Markers for mean and +-1$\sigma$')
